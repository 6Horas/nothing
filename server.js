...
// Prefixo sob o qual o site inteiro (site + /admin + /api) fica hospedado,
// já que agora ele mora junto com outras coisas no mesmo domínio pessoal.
// Ex: https://seudominio.com/wishlist-casamento/, .../wishlist-casamento/admin
const BASE_PATH = process.env.BASE_PATH || '/wishlist-casamento';

const app = express();
app.use(express.json());

// Todas as rotas (estáticas e de API) são registradas neste router,
// que depois é montado em BASE_PATH. Assim /admin (servido estaticamente
// de public/admin) automaticamente vira BASE_PATH + '/admin'.
...
router.post('/api/items', requireAdmin, (req, res) => {
  const db = req.db;
...
router.put('/api/items/:id', requireAdmin, (req, res) => {
  const db = req.db;
...
router.post('/api/items/:id/reserve', (req, res) => {
  const db = readDB();
  const item = db.items.find(i => i.id === req.params.id);
  if (!item) return res.status(404).json({ error: 'Item não encontrado.' });
...
router.post('/api/items/:id/unreserve', (req, res) => {
  const db = readDB();
  const item = db.items.find(i => i.id === req.params.id);
  if (!item) return res.status(404).json({ error: 'Item não encontrado.' });
...
// Redireciona a raiz do domínio para o site, caso alguém acesse "/" direto.
app.get('/', (req, res) => res.redirect(BASE_PATH + '/'));

app.listen(PORT, () => {
  console.log(`Servidor do Chá de Panela rodando em http://localhost:${PORT}${BASE_PATH}`);
});
